...
class EditableCell extends React.Component {
	...
	componentDidMount(){
		var cell = this.refs.editablecell;
		var self = this;
		$(cell).on("mouseenter" , function (event) {
			// console.log(event.target.index);
			$(this).find(".editable-cell-icon").css({
				"display":"block",
				"position": "absolute",
				"top" : 10,
			  	"right": 0,
		...
		$(cell).on("mouseleave" , function (event) {
...
class ShowStaff extends React.Component {
			...
			},
			{
				...
				render: (text, record, index) => {
					return (
						...
						(
							<Popconfirm title="确定删除?" onConfirm={() => this.onDelete(index,record.id)}>
							<a href="#">Delete</a>
							</Popconfirm>
						) : null
					);
	...
	componentWillReceiveProps({stafftab}){
		this.setState({
			dataSource : stafftab.staffs.data
		})
	}
	...
	render() {
		...
		return (
		<div>
		<Button style={{"marginBottom": 8}} onClick={this.handleAdd.bind(this)}>Add</Button>
		{" "}
