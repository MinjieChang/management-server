...
class PicPreview extends React.Component {
  ...
  imgClick(item, idx) {
    const index = $(this.refs.now_click).index();
    this.setState({
      picPath: item,
      index: idx,
    });
      ...
      <div className="" style={{ overflow: 'hidden' }}>
        <div>
          <img
            ...
            src={`uploads/shuoshuopic/${this.state.picPath}`}
          />
        </div>
        <div className="picNav">
          {this.state.pathArr.map((item, idx) => (
            <a
              ...
              onClick={() => {
                this.imgClick(item, idx);
              }}
            >
              <img
                src={`uploads/shuoshuopic/${item}`}
