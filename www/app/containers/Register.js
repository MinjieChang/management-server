	...
	// 点击注册按钮调用此方法
 	...
 	// 验证两次密码是否相同
  	...
  	render() {
	    ...
	    return (
	    	<div>
	      	<Form onSubmit={()=>{this.handleSubmit(this.showModal)}} style={{padding:'50px 0',margin:'0 auto',width:'650px'}}>
	        <FormItem
	          	{...formItemLayout}
	          	label = "邮箱"
	          	extra = {this.state.emailValidate ? '' : <span style={{color:'red'}}><Icon type="exclamation-circle" style={{color:'orange'}}/> 该邮箱已注册可<a onClick={this.loginClick.bind(this)}>直接登录</a></span>}
	          	hasFeedback
	        >
	        ...
	        >
	        	{getFieldDecorator('confirm', {
	            	...
	            	<Input type="password" onBlur={this.passwordConfirmBlur.bind(this)} />
	          	)}
	        </FormItem>
	        <FormItem
	          	{...formItemLayout}
	        ...
	        <FormItem {...tailFormItemLayout} style={{ marginBottom: 8 }}>
	          	{getFieldDecorator('agreement', {
	            	...
	            	<Checkbox>我已阅读<a href="">相关条款</a></Checkbox>
	          	)}
	        </FormItem>
	        ...
	        <FormItem {...tailFormItemLayout}>
	      	...
	      	<div>
        		<Modal
	          		...
	          		width='450'
        		>
          		<h3 style={{textAligh:'center'}}>{this.state.content}<Icon type="check" style={{color:'red',marginLeft:'10px',fontSize:'16px'}} /></h3>
        		</Modal>
      		</div>
	    </div>
	    );
